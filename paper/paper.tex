...
\node[courtinfo] (chief) at (0,4.6) {chief\_justice};

\node[root] (issue)  at (-5.1,2.9) {issue\_area};
\node[root] (law)    at (-1.9,2.9) {law\_type};
\node[root] (supp)   at ( 1.3,2.9) {case\_supplement};
\node[root] (lcd)    at ( 4.9,2.9) {lower\_court\_disposition};

\node[midlayer] (dtype) at (-5.1,1.2) {decision\_type};
\node[midlayer] (prec)  at (-1.9,1.2) {precedent\_alteration};
\node[midlayer] (split) at ( 1.6,1.2) {split\_vote};
\node[midlayer] (uncon) at ( 4.6,1.2) {unconstitutional};

\node[outcome] (disp) at (0,-0.6) {final\_disposition};
...
\draw[e] (split) -- (disp);
\draw[e] (uncon) -- (disp);
\draw[e] (chief) to[out=-20,in=35] (disp);

\node[font=\scriptsize\itshape, gray, anchor=west] at (-7.4,4.6) {Court info};
\node[font=\scriptsize\itshape, gray, anchor=west] at (-7.4,2.9) {Case properties};
\node[font=\scriptsize\itshape, gray, anchor=west] at (-7.4,1.2) {Intermediate};
\node[font=\scriptsize\itshape, gray, anchor=west] at (-7.4,-0.6) {Outcome};
...
\begin{description}
  ...
  \item[\code{ex\_ante}] Only the four variables knowable before the Court
    ...
    \code{lower\_court\_disposition}. The intermediate nodes become unobserved,
    so the posterior must marginalise over them.
\end{description}
...
Table~\ref{tab:convergence} confirms all three samplers converge toward the
exact posterior as the budget grows.
